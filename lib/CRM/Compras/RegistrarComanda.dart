...
class _RegistrarComandaState extends State<RegistrarComanda> {
  ...
  void _writeComandaFirebase() async {
    ...
    var dataComanda = dia.toString() +'-'+ mes.toString() +'-'+ any.toString();
    if ( mes == 12 ) mesEntrega = dia.toString() +'/'+ 1.toString() +'/'+ any.toString();
    else mesEntrega = mes.toString();
    var dataEntrega = dia.toString() +'/'+ mesEntrega +'/'+ any.toString();

    Firestore.instance.collection("comandaProveidor").document(idComanda.toString()) // amb .add( en comptes de .setData ) es pot aconseguir id automatic a FireBase, pero no es fiable
      .setData({
      'idComanda': idComanda,
